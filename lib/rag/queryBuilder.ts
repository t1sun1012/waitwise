...
import type { ConversationContext, ConversationIntent } from '../../types/rag';

export interface RetrievalQueryContextInput {
...
function buildAdjacentPhrases(tokens: string[], minSize = 2, maxSize = 3): string[] {
...
function addCorpusPhrase(
  map: Map<string, CorpusPhrase>,
  phrase: string,
  weight: number
) {
...
function buildCorpusPhraseIndex(): CorpusPhrase[] {
  const phraseMap = new Map<string, CorpusPhrase>();

  getRagCorpus().forEach((chunk) => {
    ...
    buildAdjacentPhrases(extractTopicTokens(chunk.title)).forEach((phrase) =>
      addCorpusPhrase(phraseMap, phrase, 4)
    );
  });
...
function addWeightedPhrase(
  phrases: Map<string, WeightedPhrase>,
  phrase: string,
  score: number,
  firstSeen: number
) {
...
function collectCorpusMatches(
  texts: string[],
  weight: number,
  firstSeenStart: number
): Map<string, WeightedPhrase> {
  const matches = new Map<string, WeightedPhrase>();
  let firstSeen = firstSeenStart;

  texts.forEach((text) => {
    CORPUS_PHRASES.forEach((phrase) => {
      if (!phraseInText(text, phrase.phrase)) return;
      addWeightedPhrase(
        matches,
        phrase.phrase,
        weight + phrase.weight + CORPUS_PHRASE_BONUS,
        firstSeen
      );
      firstSeen += 1;
    });
...
function sortedPhrases(values: Map<string, WeightedPhrase>): WeightedPhrase[] {
...
function pickTopPhrases(
  values: Map<string, WeightedPhrase>,
  maxCount: number
): string[] {
...
function mergeWeightedPhrases(
  target: Map<string, WeightedPhrase>,
  source: Map<string, WeightedPhrase>
) {
...
function buildFallbackWeightedQuery(
  currentPrompt: string,
  previousUserPrompts: string[],
  recentAssistantReplies: string[]
): string {
  const phrases = new Map<string, WeightedPhrase>();
  let firstSeen = 0;
  ...
  for (const phrase of sortedPhrases(phrases)) {
    const nextLength =
      totalChars + phrase.phrase.length + (parts.length > 0 ? 1 : 0);
    if (nextLength > MAX_QUERY_CHARS) continue;
...
function extractFallbackEntityPhrases(prompt: string): string[] {
...
function buildAnchorTokenSet(entities: string[], currentPrompt: string): Set<string> {
...
function collectAssistantConcepts(
  recentAssistantReplies: string[],
  ...
  const concepts = new Map<string, WeightedPhrase>();
  let firstSeen = 0;
...
function buildEntityList(
  currentMatches: Map<string, WeightedPhrase>,
  previousMatches: Map<string, WeightedPhrase>,
  currentUserPrompt: string
): string[] {
  const compactEntities = (values: string[]): string[] => {
    const selected: string[] = [];
    const coveredTokens = new Set<string>();
...
function joinQueryParts(parts: string[]): string {
  ...
  uniqueParts.forEach((part) => {
    const nextLength =
      totalChars + part.length + (selected.length > 0 ? 1 : 0);
...
function buildSummary(
  ...
  const base =
    intent === 'compare' && entities.length >= 2
...
function buildRetrievalQueriesFromContext(
  intent: ConversationIntent,
  entities: string[],
  relatedConcepts: string[],
  fallbackQuery: string
...
export function buildConversationContext(
  input: RetrievalQueryContextInput
): ConversationContext {
  const currentUserPrompt = normalizeText(input.currentUserPrompt);
  const previousUserPrompts = dedupeTexts(
    (input.recentUserPrompts ?? []).slice(-MAX_RECENT_USER_PROMPTS)
  );
  const recentAssistantReplies = dedupeTexts(
    (input.recentAssistantReplies ?? []).slice(-MAX_RECENT_ASSISTANT_REPLIES)
  );

  const currentMatches = collectCorpusMatches([currentUserPrompt], CURRENT_USER_WEIGHT, 0);
  const previousMatches = collectCorpusMatches(
    previousUserPrompts.slice().reverse(),
    PREVIOUS_USER_WEIGHTS[0],
    100
  );

  const entityCandidates = new Map<string, WeightedPhrase>();
  ...
  const fallbackQuery = buildFallbackWeightedQuery(
    currentUserPrompt,
    previousUserPrompts,
    recentAssistantReplies
  );
  ...
  const relatedConcepts = collectAssistantConcepts(
    recentAssistantReplies,
    buildAnchorTokenSet(entities, currentUserPrompt),
    entities
  );
  const summary = buildSummary(intent, entities, relatedConcepts, fallbackQuery);
  const retrievalQueries = buildRetrievalQueriesFromContext(
    intent,
    entities,
    relatedConcepts,
    fallbackQuery
