...
export interface GeminiGeneratedQuiz {
  topic: string;
  question: string;
  options: string[];
  correctIndex: number;
...
interface GenerateQuizWithGeminiParams {
  apiKey: string;
...
function clampPromptText(value: string, maxChars = MAX_PROMPT_CHARS): string {
...
export function buildGeminiPromptText(
  params: Omit<GenerateQuizWithGeminiParams, 'apiKey'>,
...
export function normalizeGeminiGeneratedQuiz(
  payload: unknown
): GeminiGeneratedQuiz | null {
  if (!payload || typeof payload !== 'object') return null;

  const candidate = payload as Partial<GeminiGeneratedQuiz>;
  if (
...
export function buildQuizQuestionFromGeminiOutput(
  generatedQuiz: GeminiGeneratedQuiz,
  retrievedChunks: RankedRetrievedChunk[]
): QuizQuestion | null {
...
export async function generateQuizWithGemini(
  params: GenerateQuizWithGeminiParams
  ...
  try {
    for (const attempt of [0, 1]) {
      ...
      console.log(
        `[wAItwise] Gemini raw output${attempt > 0 ? ' (retry)' : ''}:`,
        result.rawText ? clampDebugText(result.rawText) : result.responseJson
      );
      console.log(
        `[wAItwise] Gemini response meta${attempt > 0 ? ' (retry)' : ''}:`,
        debugMeta
      );
      ...
      console.log(
        `[wAItwise] Gemini extracted JSON payload${attempt > 0 ? ' (retry)' : ''}:`,
      ...
      console.log(
        `[wAItwise] Gemini parsed quiz${attempt > 0 ? ' (retry)' : ''}:`,
        normalizedQuiz
      );
      if (!normalizedQuiz) {
        if (attempt === 0) continue;
      ...
      return buildQuizQuestionFromGeminiOutput(
        normalizedQuiz,
